...
namespace Puzzle4B
{
    class Program
    {
        private const string Input = @"byr:2010 pid:#1bb4d8 eyr:2021 hgt:186cm iyr:2020 ecl:grt
...
eyr:2028";
        ...
        {
            var passportsRaw = Input.Split(Environment.NewLine);

            var passports = new List<string>();
            string line = string.Empty;
            foreach (var p in passportsRaw)
            {
                if (p.Trim() == string.Empty)
                {
                    passports.Add(line.Trim());
                    line = string.Empty;
                }
                else
                {
            ...
            {
                var passportFields = passport.Split(" ", StringSplitOptions.RemoveEmptyEntries);
                ...
                foreach (var field in passportFields)
                {
                    ...
                    {
                        case "byr":
                            int.TryParse(parts[1], out intValue);
                        ...
                        case "eyr":
                            ...
                            break;
                        case "hgt":
                            var amount = int.Parse(Regex.Match(parts[1], @"\d+").Value);
                            var units = parts[1].Substring(parts[1].Length - 2);
                            if (units == "cm" && amount >= 150 && amount <= 193)
                                matching++;
                            else if (units == "in" && amount >= 59 && amount <= 76)
                                matching++;
                            break;
                        case "hcl":
                            if (parts[1][0] == '#')
                            {
                                ...
                                if (color.Length == 6)
                                {
                                    if (int.TryParse(color, NumberStyles.AllowHexSpecifier, new NumberFormatInfo(), out _))
                                        matching++;
                                }
                            }
                            break;
                        case "ecl":
                            if (validEyeColors.Any(c => c == parts[1]))
                                matching++;
                            break;
                        case "pid":
                            if (parts[1].Length == 9 && int.TryParse(parts[1], out _))
                                matching++;
                            break;
                    }
                }

                if (matching == Fields.Count)
                    valid++;
            }
